...
function MergeRequestDetailPage() {
    ...
    const handleMerge = async () => {
        if (
            !window.confirm(
                "Are you sure you want to merge these changes? This cannot be undone easily.",
            )
        )
            return;
            ...
            <div className="mb-6">
                ...
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-[var(--color-text-secondary)] mb-4">
                            ...
                            <span className="font-mono bg-[var(--color-bg-tertiary)] px-1.5 py-0.5 rounded text-[var(--color-text-primary)]">
                ...
                {!canMerge && mr.status === "open" && (
                    <div className="bg-yellow-500/10 text-yellow-500 text-xs px-3 py-2 rounded mb-4 inline-block">
                        Only the target branch owner ({mr.target_branch?.name})
                        or site admin can merge this.
                    </div>
                )}
            </div>
